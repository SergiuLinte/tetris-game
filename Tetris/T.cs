...
namespace Tetris
{
    class T : Piesa
    {
        ...
        {
            game.pG_Matrice[pP_X1, pP_Y1] = 7;
            game.pG_Matrice[pP_X2, pP_Y2] = 7;
            game.pG_Matrice[pP_X3, pP_Y3] = 7;
            game.pG_Matrice[pP_X4, pP_Y4] = 7;

            for (int i = 1; i < game.pG_Matrice.GetLength(0) - 1; i++)
            {
                for (int j = 1; j < game.pG_Matrice.GetLength(1) - 1; j++)
                {
                    if (game.pG_Matrice[i, j] == 7)
                        game.tableLayoutPanel1.GetControlFromPosition(i, j).BackColor = Color.FromArgb(5,108,242);
                }
            }
        }
